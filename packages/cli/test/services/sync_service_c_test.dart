...
void main() {
  ...
  group('sync service', () {
    ...
    test('preserves platform-variant-owned inactive empty directory roots '
      ...
      final homeDirectory = p.join(workspace, 'home');
      final xdgConfigHome = p.join(workspace, 'xdg');
      final replacementFile = p.join(homeDirectory, '.config', 'variant-empty');
      final ageKeys = await createAgeKeyPair();
      setEnvironment(homeDirectory, xdgConfigHome);

      await writeIdentityFile(xdgConfigHome, ageKeys.identity);
      await Directory(p.join(homeDirectory, '.config')).create(recursive: true);
    ...
    test('preserves platform-variant-owned inactive empty directory roots '
      ...
      final homeDirectory = p.join(workspace, 'home');
      final xdgConfigHome = p.join(workspace, 'xdg');
      final replacementPath = p.join(homeDirectory, '.config', 'variant-link');
    ...
    test('ignores repository support directories when pruning removed profile '
        'namespaces', () async {
      final workspace = await createWorkspace();
      final homeDirectory = p.join(workspace, 'home');
      final xdgConfigHome = p.join(workspace, 'xdg');
    ...
    test('replaces a stale child directory with a file while the parent '
      ...
      final homeDirectory = p.join(workspace, 'home');
      final xdgConfigHome = p.join(workspace, 'xdg');
      final parentDirectory = p.join(homeDirectory, '.config', 'overlap-file');
    ...
    test('replaces a stale child directory with a symlink while the parent '
      ...
      final homeDirectory = p.join(workspace, 'home');
      final xdgConfigHome = p.join(workspace, 'xdg');
      final parentDirectory = p.join(homeDirectory, '.config', 'overlap-link');
      final childDirectory = p.join(parentDirectory, 'child');
    ...
    test('restores directory entry permission to child files and a searchable '
        'directory on pull', () async {
      if (Platform.isWindows) {
        return;
      }
    ...
    test('preserves ignored local files inside permissioned directories on '
      ...
      expect(await File(keyFile).readAsString(), 'fake-private-key\n');
      expect(await File(ignoredFile).readAsString(), 'preserved-local-state\n');
    ...
    test('deletes local files that were removed from repository during '
      ...
      expect(await File(repoPathA).readAsString(), contains('"key": "value"'));
      expect(
    ...
    test('updates repository artifacts when only the executable bit '
      ...
      await trackTarget(
        TrackRequest(mode: const TrackModeValue('normal'), target: scriptPath),
        homeDirectory,
      );
