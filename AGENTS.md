...
The Dart package and homepage have independent validation entry points:

- **All Dart checks**: `dart run tool/validate.dart`
- **Dart static checks**: `dart run tool/validate.dart dart-static`
- **Dart tests**: `dart run tool/validate.dart dart-tests`
...
For the Dart CLI package (run from the repo root):
- **Format**: `dart format .`
- **Analyze**: `dart analyze --fatal-infos`
- **Test**: `dart test`

The CLI tags its PTY-dependent shell suite, which is skipped
...
### CLI Development
- **Source Structure (under `lib/src/`):**
...
- **Parity:** Intentional behavioral divergences from the pre-cutover TypeScript implementation are recorded in `PARITY.md`.
...
## Key Files
- `lib/src/application.dart`: CLI entry point and application building.
- `lib/src/config/sync_schema.dart`: Schema for the sync configuration.
- `PARITY.md`: Recorded divergences from the pre-cutover TypeScript implementation.
