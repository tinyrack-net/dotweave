...
`repoPath` の各バリアントは、それを宣言したプラットフォームのものです。別のプラットフォームのバリアントをたまたま含むディレクトリ項目でも、その artifact を自分の内容としては扱いません。`pull` はその artifact をホームディレクトリに書き込まずリポジトリに残し、`push` は上書きも削除もしません。1 点だけ設計時に考慮してください。トラッキング中のディレクトリ内にあるローカルファイルのリポジトリパスが、別のプラットフォームの `repoPath` オーバーライドと一致する場合、そのバリアントに解決されないすべてのマシンで両方向ともスキップされます。実ファイルと衝突しないバリアント名を選ぶか、項目の `repoPath` をトラッキング対象ディレクトリの外に置いてください。
