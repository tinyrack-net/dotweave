...
モードは manifest の項目ごとに宣言され、設定の読み込み時にプラットフォーム別へ解決されます。モードの一覧は `lib/src/config/constants.dart`、プラットフォーム解決と継承は `config/sync_schema.dart`、artifact の所有権と整理は `services/repo_artifacts.dart` にあります。
