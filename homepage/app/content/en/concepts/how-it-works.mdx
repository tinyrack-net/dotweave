...
A `normal` entry is copied verbatim. A `secret` entry is encrypted to every recipient in `manifest.jsonc` and stored with a `.dotweave.secret` suffix. A symlink is stored as a `<name>.dotweave.symlink` file holding the link target, home-anchored as `~/...` when it points inside your home directory. An `ignore` entry is skipped.
