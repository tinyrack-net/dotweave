...
| --- | --- | --- |
| `--dry-run` | boolean, `false` | Build and print the plan without touching any local path. `--no-dry-run` selects the default explicitly. |
| `--profile` | `string`, none | Use one registered profile for this command instead of the active profile from `settings.jsonc`. |
| `--yes`, `-y` | boolean, `false` | Apply the plan without prompting. This flag has no `--no-yes` form. |
