...
`pull` writes only the artifacts your tracked entries own on this machine's platform key. A repository path owned by another platform's `repoPath` override stays in the repository: `pull` does not write it into your home directory, and does not delete it if it is already there. See [Platform-specific paths](/en/concepts/platform-paths/).
